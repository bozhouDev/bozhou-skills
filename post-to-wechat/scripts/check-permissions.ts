...
async function checkApiCredentials(): Promise<void> {
  const configPath = path.join(os.homedir(), '.config', 'post-to-wechat', 'config.json');
  if (fs.existsSync(configPath)) {
    try {
      const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
      if (config.app_id && config.app_secret) {
        log('API credentials', true, `Found in ${configPath}`);
      } else {
        warn('API credentials', `Config exists at ${configPath} but app_id or app_secret is empty.`);
      }
    } catch {
      warn('API credentials', `Failed to parse ${configPath}`);
    }
  } else {
    warn('API credentials', `Config not found at ${configPath}. Run the skill to set up.`);
  }
}
