...
export function loadExtendConfig(): Partial<ExtendConfig> {
  const paths = [
    path.join(process.cwd(), ".my-tools", "wechat-markdown-editor", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(homedir(), ".config"),
      "wechat-markdown-editor", "EXTEND.md"
    ),
    path.join(homedir(), ".my-tools", "wechat-markdown-editor", "EXTEND.md"),
  ];
