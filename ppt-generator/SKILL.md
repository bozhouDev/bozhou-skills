...
import sys, os
from pathlib import Path
sys.path.insert(0, str(Path(os.environ.get('SKILLS_DIR', str(Path.home() / '.claude/skills'))) / 'shared-lib'))
...
```bash
python "$SKILLS_DIR/ppt-generator/scripts/assemble_pptx.py" \
...
图片生成依赖 `$SKILLS_DIR/shared-lib/config.yaml`，包含：
