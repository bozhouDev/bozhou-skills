...
for p in [cwd, *cwd.parents]:
    s = p / '.claude/skills'
    if (s / 'illustration-generator').exists():
        print(s); exit()
...
### Step 1: 分析文章结构
```bash
python3 "$SKILLS_DIR/illustration-generator/scripts/generate.py" article.md --preview
```
这会分析文章并推荐风格，但不会生成图片。
...
### Step 4: 生成图片
```bash
python3 "$SKILLS_DIR/illustration-generator/scripts/generate.py" article.md --style <chosen_style>
```
